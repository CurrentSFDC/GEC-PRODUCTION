...
export default class PricingRequestDataTableLwc extends LightningElement() {
...
// COLUMNS FOR THE VIEW CART MODAL
    ...
    connectedCallback(){
        ...
        // Code changes for Default Product ID
    ...
    async handleClick(event){
       ...
       // var selRows = this.template.querySelector('lightning-datatable');
        ...
        var newSelVal = selected.map(row => { return { "Product_SKU__c": row.MaterialDescription__c, "Product_Description__c" :row.ccrz__ShortDescRT__c, "SKU__c":row.ccrz__SKU__c, "Type__c":"Pricing Inquiry",
                    "Comments__c": row.Comments__c,"Selected_Qty__c":row.Quantity_Selected__c,"CC_Product_Id__c":row.Id, "Transaction_ID__c": this.transactionID, "Unique_ID__c": this.transactionID+'_'+row.MaterialDescription__c }});
    ...
    showWarranty(event){
        ...
        console.log('Serach product found' +this.searchProducts);
