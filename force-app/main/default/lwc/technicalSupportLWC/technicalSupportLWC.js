...
import getTSQueueId from '@salesforce/apex/connectCreateCase.getTSQueueId';
import getQueueIdTS from '@salesforce/apex/connectCreateCase.getQueueIdTS';
import getPGCSOwnerId from '@salesforce/apex/connectCreateCase.getPGCSOwnerId';
...
import SUPPORT_TYPE_FIELD from '@salesforce/schema/Case.Support_Type__c';
import PRIMARY_PRODUCT_BRAND_FIELD from '@salesforce/schema/Case.Primary_Product_Brand__c';
...
export default class TechnicalSupportLWC extends LightningElement {
...
// COLUMNS FOR THE VIEW CART MODAL
  ...
  //  @track specRegNo;
    ...
    @track caseType = "Technical Service";
    ...
    @track selectedSoldToAccountNamecheck=false;
    ...
    @track CurrencyISOcode; //to check agent is CA$ or $
    @track CurrencyISOcodecheck=false;
...
async setAccountfromSelector(){
    ...
    if(storedUserType == "Agent"){
        // this.CurrencyISOcode =localStorage.getItem('CurrencyISOcode');
         //   console.log("Canadian agent"+ this.CurrencyISOcode);
        ...
        this.preSelectedAccount = localStorage.getItem('AgentName') + ' - ' + localStorage.getItem('AgentNumber') + ' - ' + localStorage.getItem('AgentSegment');
            ...
            console.log('agent account id '+this.selectedAccountName);
        ...
        if(retrieveData != null){
            ...
            this.soldToAccount = retrieveData;
            var inputacc = retrieveData;
                await getProdFamilies({soldToAccId: retrieveData, agentAccId: null})
                .then(result => {
                this.prodFamilies = result;
                console.log('getProdFamilies:' + result);
                })
...
async handleAccountSelected(event){
    ...
    console.log('Account id Selected from AccountLookupLWC: '+this.selectedAccountID);
...
async handleSoldToSelected(event){
    ...
    this.selectedSoldToAccountNamecheck=true;
...
get acceptedFormats() {
    return ['.agl','.doc ','.docx ',' .dwg',' .dxf','.jpeg',' .jpg',' .odt',' .pdf',' .png', '.ppt',' .pptx',' .rtf', ' .xls',' .xlsx',' .vsf'];
}

get message() {
    return ['Formats: '+'agl','doc','docx','dwg','dxf','jpeg', 'jpg','odt','pdf','png','ppt', 'pptx', 'rtf','xls', 'xlsx','vsf'  + ' Max File Size: 5 MB'];
}
...
//FOR FETCHING PICKLIST FIELD APPLICATION TYPE FROM CASE OBJECT
    ...
    // AppPicklistValues;
    @wire(getPicklistValues, { recordTypeId: '0123j000000X8ys', fieldApiName: SUPPORT_TYPE_FIELD})
    SuppPicklistValues;
    @wire(getPicklistValues, { recordTypeId: '0123j000000X8ys', fieldApiName: PRIMARY_PRODUCT_BRAND_FIELD})
    ProdPicklistValues;
...
async handlePrick(event) {
    this.ProductGroup = event.target.value;
    console.log('Product group is: '+this.ProductGroup);
    if(this.ProductGroup =='Other'){
        console.log('Product group is:' +this.ProductGroup);
    ...
    if(this.ProductGroup !== "LED Lamps & Drivers" || this.ProductGroup !== "Lamp & Ballast"){
        await getQueueIdTS({productgroup : this.ProductGroup})
        .then(result => {
            this.queuename =result;
        // console.log("The Queue name is: "+ JSON.stringify(this.queuename));   
        }
        )
    }
    console.log("The Queue name is: "+ this.queuename); 
...
//SELECTING PRODUCT DATA FROM PRODUCT DATATABLE
   /* async handleChange(event){
    ...
    await getProdItemNewList({transId: this.currentRecordId}) 
        ...
        var newSelVal = selected.map(row => { return { "Product_SKU__c": row.MaterialDescription__c, "Product_Description__c" :row.ccrz__ShortDescRT__c, "SKU__c":row.ccrz__SKU__c, "Type__c":"Lighting Design",
...
//_--------------------------------------------------------------------
 ...
 async handleSave(event){
   ...
   console.log("The Result for OwnerId: "+ JSON.stringify(this.agencyName));
    ...
    //nCase.Sold_To_Account__c = this.soldToAccount;
    if(this.selectedDistributorID != null || this.selectedDistributorID != ''){
        nCase.Sold_To_Account__c = this.selectedDistributorID;
    }
    nCase.Agent_Name__c=this.agencyManager;
    nCase.Origin = 'Connect';
    nCase.eLight_Form_Type__c = 'Technical Service';
    nCase.Connect_ID__c = this.transactionID;
    nCase.Job_Name__c = this.jobName1; 
    nCase.Primary_Product_Brand__c =  this.ProductGroup;      
  ...
  //  nCase.Spec_Reg_Number__c = this.specRegNo;
    ...
    nCase.GE_LGT_Actual_Due_Date__c = this.duedate1;   
   /// nCase.Connect_Approximate_Values__c = this.approxValues;    
    nCase.Estimated_Job_Value__c = this.estimatedJobValue;
    nCase.Comments = this.comments;
    nCase.Type = 'Technical Support';
    nCase.GE_NAS_Sub_Type__c = 'Technical Service';
    nCase.Subject =  this.subject +'- PreSales';
    nCase.Description = this.detaileddesc;
...
// FOLLOWING METHODS ARE FOR PROGRESSING THROUGH THE SCREENS
    ...
    async goToStepTwo() {
        console.log("Testt2"+this.soldToAccount);

         if(this.soldToAccount !== undefined)   {
        await getAccName({id_dtl: this.soldToAccount})
    .then(result => {
    this.acctName1 = result;
    console.log(JSON.stringify("Distributor Account Name Selected "+ JSON.stringify(this.acctName1)))
    }); 
}
        console.log("Testt1"+this.acctName1);
        this.currentStep = '2';
    ...
    // if month is less then 10, then append 0 before date    
     ...
     var todayFormattedDate = mm+'/'+dd+'/'+yyyy;
     this.duedate112= todayFormattedDate;
    ...
    duedatemap(){
     ...
     this.duedate1= todayFormattedDate1;
     console.log('Date Entered Test11: '+duedate1);
    }
    ...
    this.detaileddesc = this.template.querySelector('.fd1').value;
    console.log('Subject is'+this.detaileddesc);
        this.jobName1 = this.template.querySelector(".ln").value;
      console.log('Job Name: '+this.jobName);
      this.estimatedJobValue = this.template.querySelector(".pn").value;
      console.log('Estimated Job Value: '+this.estimatedJobValue);
        ...
        console.log("Testt"+this.accName);
        console.log("Queue name is"+this.queuename);
        console.log('PRODUCT GROUP: '+this.ProductGroup);

        if(this.ProductGroup == "Lamp & Ballast" || this.ProductGroup == "LED Lamps & Drivers"){
            this.getCaseOwner();
        } else {
            await getTSQueueId({Queuename : this.queuename})
            .then(result => {
                this.newOwnerId =result;
                console.log("Queue id from product group: "+ this.newOwnerId); 
            // console.log("Queue id from product group: "+ JSON.stringify(this.newOwnerId));   
            }
            );
        } 
    }
   ...
   // this.soldToAccount = this.template.querySelector(".stb").value;
    ...
    this.ProductGroup = this.template.querySelector(".kk").value;
   /* if(this.applicationType =='Other'){
    this.applicationType = this.template.querySelector(".ff").value;
  ...
  //  console.log('Country: '+this.EndUserAddressCountry);
    ...
    async getCaseOwner(){
        ...
        await getPGCSOwnerId({accountId : this.selectedAccountID})
        .then(result=>{
                if(result){
                    this.newOwnerId = result;
                    console.log('SETTING CASE QUEUE BASED ON ACCOUNT....'+this.newOwnerId);
                } 
            })
...
//------------------------------------------------------------------------
    ...
    beforeUnloadHandler(event){
        ...
        this.template.querySelector("c-material-lookup-container-l-w-c").clearSessionCart();  //UNCOMMENT WHEN CON-371 IS DEPLOYED
